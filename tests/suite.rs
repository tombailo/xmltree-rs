...
fn test_attribute_ns1() {
    ...
    // Search by name as a String
    assert_eq!("en", display_name_elem.get_attribute("lang".to_string()).unwrap());

    assert_eq!(None, display_name_elem.get_attribute("no_such_attribute".to_string()));
    ...
    // Search by name as a Cow
    let attribute_name_cow : Cow<'static, str> = "lang".into();
    assert_eq!("en", display_name_elem.get_attribute(attribute_name_cow).unwrap());
}
...
fn test_multi_attribute_names() {
    ...
    // TODO This is ugly
    assert_eq!("i", member_elem.take_attribute(("type", <Option<String>>::None)).unwrap());
...
//fn test_multi_element_names() {
    //let ext_ns = "http://dbus.extensions.com/schemas/dbus-extensions-v1.0";
    //let mut e = Element::parse(File::open("tests/data/multi-element-ns.xml").unwrap()).unwrap();

    //let ext_member_elem = e.take_child(("member", ext_ns)).unwrap();
    //assert_eq!(ext_member_elem.get_attribute("type").unwrap(), "i");

    //let member_elem = e.take_child(("member", None)).unwrap();
    //assert_eq!(member_elem.get_attribute("type").unwrap(), "b");
//}
